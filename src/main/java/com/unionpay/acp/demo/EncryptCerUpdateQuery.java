...
public class EncryptCerUpdateQuery {
	...
	private static Logger logger = Logger.getLogger(EncryptCerUpdateQuery.class);
		...
		if(!rspData.isEmpty()){
			if(AcpService.validate(rspData, DemoBase.encoding)){
				...
				if(("00").equals(respCode)){
					int resultCode = AcpService.updateEncryptCert(rspData);
					if (resultCode == 1) {
						logger.info("加密公钥更新成功");
					} else if (resultCode == 0) {
						logger.info("加密公钥无更新");
					} else {
						logger.info("加密公钥更新失败");
					}
