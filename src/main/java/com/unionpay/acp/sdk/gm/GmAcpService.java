...
public class GmAcpService {
	...
	private final static Logger logger = Logger.getLogger(GmAcpService.class);
	...
	public static Map<String, String> signByCertInfo(Map<String, String> reqData, String certPath, String certPwd,
		...
		try {
			if (SIGNMETHOD_SM2.equals(signMethod)) {
				data.put(param_certId, GmCertUtil.getCertIdByKeyStoreMap(certPath, certPwd));
				data.put(param_signature, GmSDKUtil.signSm2(data, certPath, certPwd, encoding));
				return data;
			}
			logger.error("未实现签名方法, version=" + version + ", signMethod=" + signMethod);
			return data;
	...
	public static String getCustomerInfoWithEncrypt(Map<String, String> customerInfoMap, String accNo,
		...
		if (!encryptedInfoSb.toString().equals("")) {
			...
			sf.append("encryptedInfo").append(EQUAL)
	...
	public static String getCardTransData(Map<String, String> cardTransDataMap, Map<String, String> requestData,
			String encoding) {
		{
			...
			if (cardTransDataMap.containsKey("track2Data")) {
				...
				track2Buffer.append(requestData.get("merId")).append(COLON)
						.append(requestData.get("orderId")).append(COLON)
						.append(requestData.get("txnTime")).append(COLON)
			...
			if (cardTransDataMap.containsKey("track3Data")) {
				...
				track3Buffer.append(requestData.get("merId")).append(COLON)
						.append(requestData.get("orderId")).append(COLON)
						.append(requestData.get("txnTime")).append(COLON)
						.append(requestData.get("txnAmt") == null ? 0 : requestData.get("txnAmt"))
	...
	public static int updateEncryptCert(Map<String, String> data) {
		return GmSDKUtil.updateEncryptCert(data.get(param_encryptPubKeyCert),
				data.get(param_certType));
	}
	...
	public static String getKVField(Map<String, String> map) {
		StringBuffer sf = new StringBuffer();
		String info = sf.append(LEFT_BRACE).append(createLinkString(map, false, false, null))
				.append(RIGHT_BRACE).toString();
		return info;
	}
