...
public class GmCertUtil{
	...
	private static void initMiddleCert() {
        ...
        if(isEmpty(path)){
            logger.warn(GmSDKConfig.SDK_MIDDLECERT_PATH + " is empty");
            return;
        }
	...
	private static void initRootCert() {
        ...
        if(isEmpty(path)){
            logger.warn(GmSDKConfig.SDK_ROOTCERT_PATH + " is empty");
            return;
        }
	...
	private static void initEncryptCert(){
        ...
        if(isEmpty(path)){
            logger.warn(GmSDKConfig.SDK_ENCRYPTCERT_PATH + " is empty");
            return;
        }
	...
	private static void initPinEncryptCert(){
        ...
        if(isEmpty(path)){
            logger.warn(GmSDKConfig.SDK_PINENCRYPTCERT_PATH + " is empty");
            return;
        }
...
//		logger.debug("read sign key from [" + certFilePath + "] pwd [" + certPwd + "]");
	...
	public static int resetEncryptCertPublicKey(String strCert) {
        ...
        // 备份成功,进行新证书的存储
        try {
            FileUtils.writeByteArrayToFile(f, strCert.getBytes());
