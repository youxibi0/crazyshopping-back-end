...
public class SDKConfig {
	...
	private final static Logger logger = Logger.getLogger(SDKConfig.class);
...
//	public static final String SDK_ENCRYPTTRACKCERT_PATH = "acpsdk.encryptTrackCert.path";
	...
	protected static Properties getPropertiesFromPath(String rootPath) {
		...
		logger.info("从路径读取配置文件: " + path);
		File file = new File(path);
		InputStream in = null;
		if (!file.exists()) {
			logger.error(rootPath + "不存在" + FILE_NAME);
			return null;
		}
		try {
	...
	protected void loadProperties1(Properties pro) {
		...
		if (!SDKUtil.isEmpty(value)) {
			...
			logger.info("配置项：6.0.0统一支付产品加密pin证书路径(这里配置的是目录，不要指定到公钥文件)==>"+SDK_PINENCRYPTCERT_PATH +"==>"+ value+" 已加载");
		}
